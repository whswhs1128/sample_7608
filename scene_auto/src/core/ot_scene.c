...
static td_s32 scene_set_pipe_static_param(td_void)
{
    ...
    ret = scene_set_pipe_param(ot_scene_set_static_acac);
    ot_scenecomm_check_return(ret, TD_FAILURE);

    ret = scene_set_pipe_param(ot_scene_set_static_local_cac);
    ot_scenecomm_check_return(ret, TD_FAILURE);

    ret = scene_set_pipe_param(ot_scene_set_static_dpc);
    ot_scenecomm_check_return(ret, TD_FAILURE);

    ret = scene_set_pipe_param(ot_scene_set_static_3dnr);
    ot_scenecomm_check_return(ret, TD_FAILURE);
...
static td_s32 scene_set_pipe_dynamic_param_part1(td_void)
{
    ...
    scene_set_dynamic_by_param_cb func_ca = ot_scene_set_dynamic_ca;
    ret = scene_set_dynamic_param_by_param(func_ca, SCENE_THREAD_TYPE_NORMAL, SCENE_DYNAMIC_CHANGE_TYPE_ISO);
    ot_scenecomm_check_return(ret, OT_SCENE_EINTER);
...
static td_s32 scene_set_pipe_dynamic_param(td_void)
{
    ...
    ot_scenecomm_check_return(ret, OT_SCENE_EINTER);
        ...
        scene_set_dynamic_by_param_cb func_hnr = ot_scene_set_dynamic_hnr;
        ret = scene_set_dynamic_param_by_param(func_hnr, SCENE_THREAD_TYPE_NORMAL, SCENE_DYNAMIC_CHANGE_TYPE_ISO);
        ot_scenecomm_check_return(ret, OT_SCENE_EINTER);
#endif
    ...
    for (td_u32 i = 0; i < g_scene_state.main_pipe_num; i++) {
        for (td_u32 j = 0; j < g_scene_state.main_pipe[i].sub_pipe_num; j++) {
            ...
            ret = ot_scene_set_dynamic_3dnr(g_scene_mode.pipe_attr[i].vcap_pipe_hdl, iso, pipe_index);
            ot_scenecomm_check_return(ret, OT_SCENE_EINTER);
        }
    }
    return TD_SUCCESS;
...
static td_s32 scene_set_pipe_wdr_dynamic_param_part1(td_void *arg)
{
    td_s32 ret;
    td_u32 i;
    ...
    for (i = 0; i < g_scene_state.main_pipe_num; i++) {
        ot_vi_pipe vi_pipe = g_scene_state.main_pipe[i].main_pipe_hdl;
        ot_scenecomm_expr_true_return(vi_pipe >= OT_SCENE_PIPE_MAX_NUM, TD_FAILURE);
        ret = ot_scene_set_dynamic_dpc_ratio(vi_pipe, g_scene_mode.pipe_attr[vi_pipe].pipe_param_index,
            g_scene_state.main_pipe[i].wdr_ratio);
        ot_scenecomm_check(ret, OT_SCENE_EINTER);
    }
    return TD_SUCCESS;
...
td_void *scene_not_linear_auto_thread(td_void *arg)
{
    ...
    while (g_scene_state.thread_not_linear.thread_flag == TD_TRUE) {
        ...
        for (i = 0; i < g_scene_state.main_pipe_num; i++) {
            ot_vi_pipe vi_pipe = g_scene_state.main_pipe[i].main_pipe_hdl;
            ot_scenecomm_expr_true_return(vi_pipe >= OT_SCENE_PIPE_MAX_NUM, NULL);
        }

        scene_set_pipe_wdr_dynamic_param_part1(arg);
