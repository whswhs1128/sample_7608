...
static void print_para_n(const ot_vpss_nrx_v2 *p_x, const ot_vpss_nrx_v2_iey *pi, const ot_vpss_nrx_v2_sfy *ps,
    const ot_vpss_nrx_v2_tfy *pt)
{
...
static void print_nc0(const ot_vpss_nrx_v2 *p_x, const ot_vpss_nrx_v2_tfy *pt, const ot_vpss_nrx_v2_nrc1 *pc,
    const ot_vpss_nrx_v2_mdy *pm)
{
    printf("\n -ref            %3d        |        %3d         |                   "
...
static void print_nc1(const ot_vpss_nrx_v2 *p_x, const ot_vpss_nrx_v2_tfy *pt, const ot_vpss_nrx_v2_nrc1 *pc,
    const ot_vpss_nrx_v2_mdy *pm)
{
    printf("\n -nXtdz          %3d        |        %3d:%3d     |        %3d        "
        "|********************nr_c1****************",
...
static void print_nrx(const ot_vpss_nrx_v2 *p_x)
{
    const ot_vpss_nrx_v2_iey *pi = p_x->iey;
    const ot_vpss_nrx_v2_sfy *ps = p_x->sfy;
    const ot_vpss_nrx_v2_mdy *pm = p_x->mdy;
    const ot_vpss_nrx_v2_tfy *pt = p_x->tfy;
    const ot_vpss_nrx_v2_nrc1 *pc = &p_x->nrc1;
    ...
    ot_scene_dynamic_drc_coef *drc_coef, ot_isp_drc_attr *isp_drc_attr)
{
    ...
    if (drc_coef->ref_exp_ratio >= (g_scene_pipe_param[index].static_wdr_exposure.exp_ratio_max - 16)) { /* sub16 */
        for (i = 0; i < OT_ISP_DRC_TM_NODE_NUM; i++) {
            tonemapping_value[i] =
            ((td_u32)blend_alpha * g_scene_pipe_param[index].dynamic_drc.tm_val_higher[i] +
             /* 0x200 blend_alpha, precison 9bit */
            (td_u32)(0x200 - blend_alpha) * g_scene_pipe_param[index].dynamic_drc.tm_val_highest[i]) >> 9; /* shift9 */
        }
...
td_s32 scene_set_tone_mapping_value(ot_vi_pipe vi_pipe, td_u8 index, td_u32 k,
    ot_isp_inner_state_info inner_state_info, ot_isp_drc_attr *isp_drc_attr)
{
    static td_u32 ref_exp_ratio[OT_SCENE_PIPETYPE_NUM][OT_SCENE_REF_EXP_RATIO_FRM] = {0};
    ...
    ot_scenecomm_expr_true_return(vi_pipe >= OT_SCENE_PIPETYPE_NUM, TD_FAILURE);
...
td_s32 scene_set_isp_drc_attr(td_u8 index, td_u64 iso, ot_isp_drc_attr *isp_drc_attr)
{
    ...
    for (td_u32 i = 0; i < OT_ISP_DRC_FLTX_NODE_NUM; i++) {
...
td_s32 scene_set_nr_wdr_ratio_para(ot_vi_pipe vi_pipe, td_u8 index, td_u32 wdr_ratio, ot_isp_nr_attr *nr_attr)
{
    td_u32 i;
    td_s32 ret;
...
static td_void scene_set_nry_en(ot_vpss_grp_param *nrx_attr, const ot_scene_3dnr *_3dnr)
{
...
static td_void scene_set_nry_iey(ot_vpss_grp_param *nrx_attr, const ot_scene_3dnr *_3dnr)
{
    /* ie_y */
    for (td_u32 i = 0; i < ARRAY_SIZE(_3dnr->iey); i++) {
...
static td_void scene_set_nry_sfy_1to4(ot_vpss_grp_param *nrx_attr, const ot_scene_3dnr *_3dnr)
{
    /* sf_y */
    for (td_u32 i = 0; i < 4; i++) { /* array sfy have 4 valid element */
        /* sfs1 */
...
static td_void scene_set_nry_sfy_6to8(ot_vpss_grp_param *nrx_attr, const ot_scene_3dnr *_3dnr)
{
    for (td_u32 i = 0; i < ARRAY_SIZE(_3dnr->sfy); i++) {
        /* sfs7 */
        ...
        /* sfr7 */
        for (td_u32 j = 0; j < ARRAY_SIZE(_3dnr->sfy[i].sfr7); j++) {
    ...
    /* sf8 */
    for (td_u32 i = 3; i <= 4; i++) { /* sfy array index 3 4 */
    ...
    /* bri */
    for (td_u32 i = 2; i <= 3; i++) { /* sfy array index 2 3 */
...
static td_void scene_set_nry_tfy(ot_vpss_grp_param *nrx_attr, const ot_scene_3dnr *_3dnr)
{
    /* ref */
    ...
    /* tf_y */
    for (td_u32 i = 0; i < ARRAY_SIZE(_3dnr->tfy); i++) {
        /* str */
        ...
        /* tfr0 */
        for (td_u32 j = 0; j < ARRAY_SIZE(_3dnr->tfy[i].tfr0); j++) {
            nrx_attr->nrx_param.nrx_norm_param_v2.nrx_manual.nrx_param.tfy[i].tfr0[j] = _3dnr->tfy[i].tfr0[j];
        }

        /* tfr1 */
        for (td_u32 j = 0; j < ARRAY_SIZE(_3dnr->tfy[1].tfr1); j++) {
            nrx_attr->nrx_param.nrx_norm_param_v2.nrx_manual.nrx_param.tfy[1].tfr1[j] = _3dnr->tfy[1].tfr1[j];
        }
    }
}

static td_void scene_set_nry_mdy(ot_vpss_grp_param *nrx_attr, const ot_scene_3dnr *_3dnr)
{
    /* md_y */
    for (td_u32 i = 0; i < ARRAY_SIZE(_3dnr->mdy); i++) {
        /* id0 */
...
static td_void scene_set_nry_nc0(ot_vpss_grp_param *nrx_attr, const ot_scene_3dnr *_3dnr)
{
    /* nrc0_en */
...
static td_void scene_set_nry_nc1(ot_vpss_grp_param *nrx_attr, const ot_scene_3dnr *_3dnr)
{
    /* nrc1_en */
    ...
    for (td_u32 i = 0; i < OT_SCENE_3DNR_IDX_LEN; i++) { /* 2 3 sfy index */
...
td_s32 scene_set_3dnr(ot_vi_pipe vi_pipe, const ot_scene_3dnr *_3dnr, td_u8 index)
{
    ...
    ot_vpss_grp_param nrx_attr, nrx_get_attr;
    nrx_attr.nrx_param.nr_version = OT_VPSS_NR_V2;
    nrx_attr.nrx_param.nrx_norm_param_v2.op_mode = OT_OP_MODE_MANUAL;
    ret = ss_mpi_vpss_get_grp_param(vi_pipe, &nrx_attr);
    check_scene_ret(ret);
    ...
    ret = ss_mpi_vpss_set_grp_param(vi_pipe, &nrx_attr);
    check_scene_ret(ret);

    nrx_get_attr.nrx_param.nr_version = OT_VPSS_NR_V2;
    nrx_get_attr.nrx_param.nrx_norm_param_v2.op_mode = OT_OP_MODE_MANUAL;
    ret = ss_mpi_vpss_get_grp_param(vi_pipe, &nrx_get_attr);
    check_scene_ret(ret);

    static td_u32 count[OT_SCENE_PIPETYPE_NUM] = {0};
...
td_void scene_set_3dnr_nrx_iey(_3dnr_nrx_param param)
{
    ...
    /* ie_y */
    for (td_u32 i = 0; i < 5; i++) { /* 5 iey array size */
...
static td_void scene_set_nrx_sfy(_3dnr_nrx_param param)
{
    ...
    /* sf_y */
    for (td_u32 i = 0; i < 4; i++) { /* 4 sfy index */
        /* sfs1 */
...
static td_void scene_set_nrx_sfy4(_3dnr_nrx_param param)
{
    ot_scene_3dnr *nrx_attr = param.nrx_attr;
    const ot_scene_static_3dnr *nrx_param = param.nrx_param;
    td_u32 iso_level = param.iso_level;
...
static td_void set_sfs7_param(_3dnr_nrx_param param)
{
    ...
    for (td_u32 i = 0; i < ARRAY_SIZE(nrx_attr->sfy); i++) {
        /* sfs7 */
        ...
        /* sfr7 */
        for (td_u32 j = 0; j < ARRAY_SIZE(nrx_attr->sfy[i].sfr7); j++) {
...
static td_void scene_set_nrx_sfy8(_3dnr_nrx_param param)
{
    ot_scene_3dnr *nrx_attr = param.nrx_attr;
    const ot_scene_static_3dnr *nrx_param = param.nrx_param;
    td_u32 iso_level = param.iso_level;
...
td_void scene_set_3dnr_nrx_tfy(_3dnr_nrx_param param)
{
    ot_scene_3dnr *nrx_attr = param.nrx_attr;
    const ot_scene_static_3dnr *nrx_param = param.nrx_param;
    td_u32 iso_level = param.iso_level;
    ...
    /* tf_y */
    for (td_u32 i = 0; i < ARRAY_SIZE(nrx_attr->tfy); i++) {
        /* str */
        ...
        /* tfr0 */
        for (td_u32 j = 0; j < ARRAY_SIZE(nrx_attr->tfy[i].tfr0); j++) {
            scene_set_nrx_attr_interpulate(tfy[i].tfr0[j]);
        }

        /* tfr1 */
        for (td_u32 j = 0; j < ARRAY_SIZE(nrx_attr->tfy[1].tfr1); j++) { /* tfr1 array size 6 */
...
td_void scene_set_3dnr_nrx_mdy(_3dnr_nrx_param param)
{
    ...
    /* md_y */
    for (td_u32 i = 0; i < ARRAY_SIZE(nrx_attr->mdy); i++) {
        /* id0 */
...
static td_void set_nrc_sfs_attr(_3dnr_nrx_param param)
{
    ot_scene_3dnr *nrx_attr = param.nrx_attr;
    const ot_scene_static_3dnr *nrx_param = param.nrx_param;
    td_u32 iso_level = param.iso_level;
