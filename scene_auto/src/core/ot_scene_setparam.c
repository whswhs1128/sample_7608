...
td_s32 ot_scene_set_static_ccm(ot_vi_pipe vi_pipe, td_u8 index)
{
    ...
    for (td_u32 i = 0; i < get_pipe_params()[index].static_ccm.total_num; i++) {
...
{
    ot_scenecomm_expr_true_return(index >= OT_SCENE_PIPETYPE_NUM, TD_FAILURE);
    if (get_pipe_params()[index].module_state.static_acac != TD_TRUE) {
        return TD_SUCCESS;
    }
    td_u32 i, j;
    td_s32 ret;
    ot_isp_acac_attr acac_attr;

    ret = ss_mpi_isp_get_acac_attr(vi_pipe, &acac_attr);
    check_scene_ret(ret);
    ...
    ret = ss_mpi_isp_set_acac_attr(vi_pipe, &acac_attr);
    check_scene_ret(ret);
    return TD_SUCCESS;
}
...
{
    ot_scenecomm_expr_true_return(index >= OT_SCENE_PIPETYPE_NUM, TD_FAILURE);
    if (get_pipe_params()[index].module_state.static_local_cac != TD_TRUE) {
        return TD_SUCCESS;
    }

    td_s32 i, ret;
    ot_isp_local_cac_attr local_cac;

    ret = ss_mpi_isp_get_local_cac_attr(vi_pipe, &local_cac);
    check_scene_ret(ret);
    ...
    ret = ss_mpi_isp_set_local_cac_attr(vi_pipe, &local_cac);
    check_scene_ret(ret);
    return TD_SUCCESS;
}
...
td_s32 ot_scene_set_dynamic_ae(ot_vi_pipe vi_pipe, td_u64 exposure, td_u64 last_exposure, td_u8 index)
{
    ...
    td_u32 actual_ratio;
    static td_u32 last_ratio[MAX_ISP_PIPE_NUM] = {[0 ... MAX_ISP_PIPE_NUM - 1] = 0x40};
    ot_isp_exposure_attr exposure_attr;
...
td_s32 ot_scene_set_dynamic_dpc(ot_vi_pipe vi_pipe, td_u64 iso, td_u64 last_iso, td_u8 index)
{
    ...
    if ((iso_level == (iso_count - 1)) || (iso_level == 0)) {
        ...
        dp_dynamic_attr.soft_thr = (td_s8)get_pipe_params()[index].dynamic_dpc.soft_thr[iso_level];
...
{
    ot_scenecomm_expr_true_return(index >= OT_SCENE_PIPETYPE_NUM, TD_FAILURE);
    if (get_pipe_params()[index].module_state.dynamic_dpc != TD_TRUE) {
        return TD_SUCCESS;
    }
    ...
    ot_isp_dp_dynamic_attr dp_dynamic_attr;
    td_u32 ratio_index;
    td_u32 ratio_count = get_pipe_params()[index].dynamic_dpc.ratio_count;
    td_u32 *level = get_pipe_params()[index].dynamic_dpc.ratio_level;
    ...
    ratio_index = scene_get_level_ltoh_u32(wdr_ratio, ratio_count, level);
    ot_scenecomm_expr_true_return(ratio_index >= ISP_AUTO_ISO_DPC_NUM, TD_FAILURE);
...
td_s32 ot_scene_set_dynamic_video_gamma(ot_vi_pipe vi_pipe, td_u64 exposure, td_u64 last_exposure, td_u8 index)
{
    ...
    td_u32 i, j;
    td_u32 exp_level;
    static td_u32 last_exp_level[OT_SCENE_PIPETYPE_NUM] = {0};
    td_s32 ret;
...
{
    ot_scenecomm_expr_true_return(index >= OT_SCENE_PIPETYPE_NUM, TD_FAILURE);
    if ((get_pipe_params()[index].module_state.dynamic_hnr != TD_TRUE) || (last_iso == iso)) {
        return TD_SUCCESS;
    }

    td_s32 ret;
    ot_isp_dp_dynamic_attr dp_dynamic_attr;
    ot_hnr_attr isp_hnr_attr;

    td_u32 *dpc_iso_thresh = get_pipe_params()[index].dynamic_hnr.dpc_iso_thresh;
    td_u32 *hnr_iso_thresh = get_pipe_params()[index].dynamic_hnr.hnr_iso_thresh;
    td_bool dpc_chg_en = get_pipe_params()[index].dynamic_hnr.dpc_chg_en;
    td_bool hnr_chg_en = get_pipe_params()[index].dynamic_hnr.hnr_chg_en;
    check_scene_return_if_pause();
    ret = ss_mpi_hnr_get_attr(vi_pipe, &isp_hnr_attr);
    check_scene_ret(ret);
    ret = ss_mpi_isp_get_dp_dynamic_attr(vi_pipe, &dp_dynamic_attr);
    check_scene_ret(ret);

    if ((iso >= hnr_iso_thresh[1]) && (hnr_chg_en == TD_TRUE)) {
        isp_hnr_attr.enable = TD_TRUE;
    }

    if ((iso <= hnr_iso_thresh[0]) && (hnr_chg_en == TD_TRUE)) {
        isp_hnr_attr.enable = TD_FALSE;
    }

    if ((iso >= dpc_iso_thresh[1]) && (dpc_chg_en == TD_TRUE)) {
        dp_dynamic_attr.enable = TD_FALSE;
    }

    if ((iso <= dpc_iso_thresh[0]) && (dpc_chg_en == TD_TRUE)) {
        dp_dynamic_attr.enable = TD_TRUE;
    }

    ret = ss_mpi_hnr_set_attr(vi_pipe, &isp_hnr_attr);
    check_scene_ret(ret);
    ret = ss_mpi_isp_set_dp_dynamic_attr(vi_pipe, &dp_dynamic_attr);
    check_scene_ret(ret);
    check_scene_return_if_pause();
...
static td_bool g_fpn_enable[OT_SCENE_PIPETYPE_NUM] = {TD_FALSE};
...
td_s32 ot_scene_set_dynamic_fpn(ot_vi_pipe vi_pipe, td_u64 iso, td_u64 last_iso, td_u8 index)
{
    ...
    static td_bool last_iso_tmp[OT_SCENE_PIPETYPE_NUM] = {0};
    ot_isp_exp_info exp_info;
    td_u32 iso_count = get_pipe_params()[index].dynamic_fpn.iso_count;
    td_u32 *iso_level_thresh = get_pipe_params()[index].dynamic_fpn.iso_thresh;
...
td_s32 ot_scene_set_rc_param(ot_vi_pipe vi_pipe, td_u8 idx)
{
    ...
    pic_width = (type == OT_PT_H264) ? venc_chn_attr.venc_attr.pic_width : venc_chn_attr.venc_attr.pic_width;
    pic_height = (type == OT_PT_H264) ? venc_chn_attr.venc_attr.pic_height : venc_chn_attr.venc_attr.pic_height;
