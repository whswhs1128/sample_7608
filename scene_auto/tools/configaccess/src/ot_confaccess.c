...
/* cfg list */
static ot_list_head g_config_access_list = ot_list_init_head_define(g_config_access_list);
...
{
    ot_list_head *pos = NULL;
    conf_access_module *module = NULL;
    td_s32 module_cnt = 0;

    ot_list_for_each(pos, &(cfg_node->module.list)) {
        ot_list_del(pos);
        module = ot_list_entry((unsigned long)(uintptr_t)pos, conf_access_module, list);
        scene_logd("Module: inifile[%s]\n", module->ini_file);
        module_cnt++;
        iniparser_freedict(module->ini_dir);
        module->ini_dir = NULL;
...
static td_s32 confaccess_get_all_modules_info(td_s32 module_num, conf_access_node *cfg_pos, const td_char *cfg_path_buf)
{
    ...
    for (td_s32 index = 0; index < module_num; ++index) {
        ...
        const td_char *value = iniparser_getstring(cfg_pos->module.ini_dir, (const char *)module_node_key, NULL);
        if (value == NULL) {
        ...
        value = iniparser_getstring(cfg_pos->module.ini_dir, (const char *)module_node_key, NULL);
        if (value == NULL) {
            scene_loge("Load %s failed\n", module_node_key);
            ot_scenecomm_safe_free(module_info);
            continue;
        ...
        snprintf_truncated_s(module_info->ini_file, OT_CONFACCESS_PATH_MAX_LEN, "%s%c%s", module_info->path,
            CONFACCESS_PATH_SEPARATOR, value);

        /* Module IniDir */
        module_info->ini_dir = iniparser_load(module_info->ini_file);
...
static td_s32 confaccess_init(const td_char *cfg_name, const td_char *cfg_path, td_u32 *out_module_num)
{
    ...
    cfg_pos->module.ini_dir = iniparser_load(cfg_path);
    ...
    *out_module_num = (td_u32)iniparser_getint(cfg_pos->module.ini_dir, (td_char *)"module:module_num", 0);
#ifndef __LITEOS__
    scene_logd("ModuleNum[%d], DefaultPath[%s]\n", (*out_module_num), pazModuleDefPath);
#endif
    ...
    if (ret != TD_SUCCESS) {
        ...
        iniparser_freedict(cfg_pos->module.ini_dir);
        goto exit;
    }
    ...
    g_conf_access_count++;
    scene_logd("CfgName[%s] ModuleCnt[%d] Count[%d]\n", cfg_pos->cfg_name, module_cnt, g_conf_access_count);
    return TD_SUCCESS;
...
td_s32 ot_confaccess_init(const td_char *cfg_name, const td_char *cfg_path, td_u32 *module_num)
{
    ...
    ot_list_for_each(pos, &(g_config_access_list)) {
        cfg_node = ot_list_entry((unsigned long)(uintptr_t)pos, conf_access_node, list);
...
td_s32 ot_confaccess_deinit(const td_char *cfg_name)
{
    ...
    ot_list_head *node_pos = NULL;
    ot_list_head *module_pos = NULL;
    conf_access_node *cfg_node = NULL;
    conf_access_module *cfg_module = NULL;
    td_s32 module_cnt = 0;

    ot_list_for_each(node_pos, &(g_config_access_list)) {
        cfg_node = ot_list_entry((unsigned long)(uintptr_t)node_pos, conf_access_node, list);
        if (strncmp(cfg_node->cfg_name, cfg_name, OT_CONFACCESS_NAME_MAX_LEN) == 0) {
            ...
            ot_list_for_each(module_pos, &(cfg_node->module.list)) {
                ...
                cfg_module = ot_list_entry((unsigned long)(uintptr_t)module_pos, conf_access_module, list);
                module_cnt++;
                iniparser_freedict(cfg_module->ini_dir);
                cfg_module->ini_dir = NULL;
                ot_scenecomm_safe_free(cfg_module);
                module_pos = &(cfg_node->module.list);
            }
            scene_logd("CfgName[%s] ModuleCnt[%d]\n", cfg_node->cfg_name, module_cnt);
            iniparser_freedict(cfg_node->module.ini_dir);
        ...
        if (strncmp(cfg_node->cfg_name, cfg_name, OT_CONFACCESS_NAME_MAX_LEN) == 0) {
            ...
            ot_list_for_each(module_pos, &(cfg_node->module.list)) {
                ...
                if (strncmp(cfg_module->name, module, OT_CONFACCESS_NAME_MAX_LEN) == 0) {
                    *value_vector = (td_char *)iniparser_getstring(cfg_module->ini_dir, conf_item, default_val);
    ...
    ot_list_for_each(node_pos, &(g_config_access_list)) {
        ...
        if (strncmp(cfg_node->cfg_name, cfg_name, OT_CONFACCESS_NAME_MAX_LEN) == 0) {
            ...
            ot_list_for_each(module_pos, &(cfg_node->module.list)) {
                cfg_module = ot_list_entry((unsigned long)(uintptr_t)module_pos, conf_access_module, list);
                if (strncmp(cfg_module->name, module, OT_CONFACCESS_NAME_MAX_LEN) == 0) {
                    if (iniparser_set(cfg_module->ini_dir, conf_item, value) != 0) {
