...
td_s32 sample_svp_npu_get_input_data(const td_char *src[], td_u32 file_num, const sample_svp_npu_task_info *task)
{
    td_s32 ret;
...
td_s32 sample_svp_npu_load_model(const td_char *model_path, td_u32 model_index, td_bool is_cached)
{
    td_s32 ret;
...
td_void sample_svp_npu_unload_model(td_u32 model_index)
{
    svp_acl_error ret;
...
static svp_acl_data_buffer *sample_svp_npu_create_input_data_buffer(sample_svp_npu_task_info *task, td_u32 idx)
{
...
static svp_acl_data_buffer *sample_svp_npu_create_output_data_buffer(const sample_svp_npu_task_info *task, td_u32 idx)
{
    size_t buffer_size, stride;
...
td_s32 sample_svp_npu_create_input(sample_svp_npu_task_info *task)
{
    svp_acl_error ret;
    td_u32 i;
    svp_acl_data_buffer *input_data = TD_NULL;
    ...
    for (i = 0; i < g_svp_npu_model[task->cfg.model_idx].input_num - SAMPLE_SVP_NPU_EXTRA_INPUT_NUM; i++) {
        ...
        if (input_data == TD_NULL) {
            ...
            (td_void)sample_svp_npu_destroy_input(task);
            return TD_FAILURE;
        }
...
td_s32 sample_svp_npu_create_output(sample_svp_npu_task_info *task)
{
    svp_acl_error ret;
    td_u32 i;
    svp_acl_data_buffer *output_data = TD_NULL;
    ...
    for (i = 0; i < g_svp_npu_model[task->cfg.model_idx].output_num; i++) {
        ...
        if (ret != SVP_ACL_SUCCESS) {
            ...
            (td_void)sample_svp_npu_destroy_output(task);
            return TD_FAILURE;
        }
    }
...
td_void sample_svp_npu_destroy_input(sample_svp_npu_task_info *task)
{
    td_u32 i;
    size_t input_num;
    svp_acl_data_buffer *data_buffer = TD_NULL;
...
td_void sample_svp_npu_destroy_output(sample_svp_npu_task_info *task)
{
    td_u32 i;
    size_t output_num;
    svp_acl_data_buffer *data_buffer = TD_NULL;
...
td_void sample_svp_npu_destroy_task_buf(sample_svp_npu_task_info *task)
{
    if (sample_svp_check_task_cfg(task) != TD_SUCCESS) {
        sample_svp_trace_err("check task cfg failed!\n");
        return;
...
td_void sample_svp_npu_destroy_work_buf(sample_svp_npu_task_info *task)
{
    if (sample_svp_check_task_cfg(task) != TD_SUCCESS) {
        sample_svp_trace_err("check task cfg failed!\n");
        return;
...
td_s32 sample_svp_npu_create_task_buf(sample_svp_npu_task_info *task)
{
    size_t num;
...
td_s32 sample_svp_npu_create_work_buf(sample_svp_npu_task_info *task)
{
    size_t num;
...
td_s32 sample_svp_npu_get_work_buf_info(const sample_svp_npu_task_info *task,
...
td_s32 sample_svp_npu_share_work_buf(const sample_svp_npu_shared_work_buf *shared_work_buf,
...
td_void sample_svp_npu_output_classification_result(const sample_svp_npu_task_info *task)
{
    svp_acl_data_buffer *data_buffer = TD_NULL;
    td_void *data = TD_NULL;
    td_u32 i, j, n;
...
td_s32 sample_svp_npu_set_dynamic_batch(const sample_svp_npu_task_info *task)
{
    svp_acl_error ret;
...
td_s32 sample_svp_npu_model_execute(const sample_svp_npu_task_info *task)
{
    svp_acl_data_buffer *data_buffer = TD_NULL;
    td_void *data = TD_NULL;
    size_t size;
...
td_void sample_svp_npu_dump_task_data(const sample_svp_npu_task_info *task)
{
    if (sample_svp_check_task_cfg(task) != TD_SUCCESS) {
        sample_svp_trace_err("check task cfg failed!\n");
        return;
...
td_s32 sample_svp_npu_update_input_data_buffer_info(td_u8 *virt_addr, td_u32 size, td_u32 stride, td_u32 idx,
    const sample_svp_npu_task_info *task)
{
    svp_acl_data_buffer *data_buffer = TD_NULL;
    svp_acl_error ret;
...
td_s32 sample_svp_npu_get_input_data_buffer_info(const sample_svp_npu_task_info *task, td_u32 idx,
...
td_s32 sample_svp_npu_roi_to_rect(const sample_svp_npu_task_info *task, sample_svp_npu_detection_info *detection_info,
    const ot_video_frame_info *proc_frame, const ot_video_frame_info *show_frame, ot_sample_svp_rect_info *rect_info)
{
    td_s32 ret;
...
td_s32 sample_svp_npu_set_threshold(sample_svp_npu_threshold threshold[], td_u32 threshold_num,
    const sample_svp_npu_task_info *task)
{
    td_u32 i, n;
    svp_acl_error ret;
...
td_s32 sample_svp_npu_check_has_aicpu_task(const sample_svp_npu_task_info *task, td_bool *has_aicpu_task)
{
